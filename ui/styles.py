...
class StyleManager:
    ...
    def _configure_styles(self):
        self.style.configure(
            "TFrame", background=self.BG_MAIN)
        self.style.configure(
            "Sidebar.TFrame", background=self.BG_SIDEBAR, 
            relief="flat")
        self.style.configure(
            "TLabel", background=self.BG_MAIN, 
            foreground=self.TEXT, font=(self.FONT_FAMILY, 10))
        self.style.configure(
            "Sidebar.TLabel", background=self.BG_SIDEBAR,
             foreground="white", font=(self.FONT_FAMILY, 10))
        self.style.map(
            "Sidebar.TLabel", 
