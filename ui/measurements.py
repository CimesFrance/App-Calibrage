...
radio pour sélectionner la mesure active)"""
    ...
    def mesure_GUI(self):
        if self.mesure_frame:
            ...
            top.pack(fill="x")
            self.title_label = tk.Label(top, text=self.title, font=("Segoe UI", 9, "bold"), 
                     bg="#FFFFFF", fg="#212529")
            ...
            self.lbl_val = tk.Label(dash, textvariable=self.longueur, 
                                    font=("Consolas", 14, "bold"), bg="#F8F9FA", fg="#0056B3")
            self.lbl_val.pack(side="left", padx=10)
            tk.Label(dash, text="mm", font=("Segoe UI", 8), bg="#F8F9FA", fg="#ADB5BD").pack(side="right", padx=10)
            # Contrôles discrets
            btm = tk.Frame(self.mesure_frame, bg="#FFFFFF")
            btm.pack(fill="x")
            tk.Checkbutton(btm, text="Afficher", variable=self.flag_affiche_ptLigne, 
                           bg="#FFFFFF", font=("Segoe UI", 8)).pack(side="left")
            tk.Radiobutton(btm, text="Saisir", value=self.num, variable=self.app.choix_mesure, 
                            bg="#FFFFFF", font=("Segoe UI", 8)).pack(side="right")
...
class mesures_supp():
    ...
    def mesures_supp_GUI(self):
        if self.mesures_supp_frame is not None:
            ...
            self.btn_ajouter = ttk.Button(self.mesures_supp_frame, text='Ajouter', command=self._ajouter_mesure)
            self.btn_supprimer = ttk.Button(self.mesures_supp_frame, text='Supprimer', command=self._supprimer_mesure)
            ...
            for i, (cle, mesure) in enumerate(self.mes_mesures_supp.items(), start=3):
                mesure.mesure_GUI()
                mesure.mesure_frame.grid(row=i, column=0, columnspan=2, sticky="nsew", padx=2, pady=2)
