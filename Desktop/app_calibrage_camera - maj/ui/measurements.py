...
radio pour sélectionner la mesure active)"""
    ...
    def mesure_GUI(self):
        if self.mesure_frame:
            ...
            self.lbl_val = ttk.Label(self.dash, textvariable=self.longueur, style="Value.TLabel")
            ...
            btm.pack(fill="x")
            self.check_affichage = ttk.Checkbutton(btm, text="Afficher", variable=self.flag_affiche_ptLigne, 
                           style="Sidebar.TCheckbutton", command=self._affiche_mesure)
            self.check_affichage.pack(side="left")
            self.radio_saisir = ttk.Radiobutton(btm, text="Saisir", value=self.num, variable=self.app.choix_mesure, 
    ...
    def display_state(self, *args):
        ...
        if hasattr(self, 'title_label') and self.title_label:
            ...
            self.radio_saisir.config(state="normal") # Toujours actif pour cibler l'ajout
...
class mesures_supp():
    ...
    def mesures_supp_GUI(self):
        if self.mesures_supp_frame is not None:
            self.btn_ajouter = ttk.Button(self.mesures_supp_frame, text='Ajouter', command=self._ajouter_mesure, style="TButton")
            self.btn_supprimer = ttk.Button(self.mesures_supp_frame, text='Supprimer', command=self._supprimer_mesure, style="Secondary.TButton")
