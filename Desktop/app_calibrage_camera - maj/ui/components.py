...
class image_frame(Card):
    ...
    def _build(self):
        ...
        f.pack(fill="x")
        ttk.Label(f, text="Sélectionnez une image", style="Sidebar.TLabel").pack(side="left")
        ttk.Button(f, image=self.tk_charger_logo, command=self._import_tk_img, 
...
class Echelle_frame(Card):
    ...
    def _build(self):
        ...
        self.lbl = ttk.Label(self.input_f, text="Longueur réelle (mm)", style="Sidebar.TLabel")
        self.lbl.pack(side="left")
        self.ent = ttk.Entry(self.input_f, textvariable=self.app.distance_saisie, width=10)
        self.ent.pack(side="right")
        self.btn = ttk.Button(self, text="Appliquer l'Échelle", 
                              command=self.apply_scale, style="TButton")
...
class Interraction(tk.Frame):
    ...
    def __init__(self, parent, app, **kwargs):
        ...
        self.app = app
        self.canvas = tk.Canvas(self, bg=self.bg_color, highlightthickness=0, width=width)
        ...
        self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw", width=width)
    ...
    def _setup_measurements(self):
        self.app.mesures_supp.mesures_supp_frame = tk.Frame(self.scrollable_frame, bg=self.bg_color)
        for m in self.app.mesures_supp.mes_mesures_supp.values():
            m.mesure_frame = tk.Frame(self.app.mesures_supp.mesures_supp_frame, bg=self.bg_color)

    def interraction_GUI(self):
        image_frame(self.scrollable_frame, self.app).pack(fill="x", padx=15, pady=(10, 5))
        tk.Frame(self.scrollable_frame, height=1, bg="#34495E").pack(fill="x", padx=15, pady=15)
        Echelle_frame(self.scrollable_frame, self.app).pack(fill="x", padx=15, pady=5)
        tk.Frame(self.scrollable_frame, height=1, bg="#34495E").pack(fill="x", padx=15, pady=15)
        self.m_card = Card(self.scrollable_frame, title="Mesures de contrôle")
        self.m_card.pack(fill="x", padx=15, pady=5)
        self.app.mesures_supp.mesures_supp_frame.pack(fill="x", padx=15, pady=(0, 10))
